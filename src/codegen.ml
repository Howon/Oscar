...
let translate (messages, actors, functions) =
  ...
  let rec ltype_of_typ (typ : A.types) = match typ with
    ...
    | A.Func_t (typs, rt) -> L.ltype_of_func typs rt
  ...
  (*let function_decls =
    ...
    (* takes a build_expr, which is (sexpr, types) *)
  ...
  let build_function_body func =
    ...
    let () =  build_stmt builder func.S.sf_body in
  ...
  and build_fbody builder func f =
    let () = L.position_at_end (L.entry_block func) builder in

    let () = ignore(List.map2 (add_local builder) f.sf_formals
                      (Array.to_list (L.params func))) in
    let () = build_stmt builder f.sf_body in

    match f.sf_return_t with
        A.Unit_t -> ignore (add_terminal builder L.build_ret_void)
      | _ -> ()

  let rec build_func builder func =
  ...
  let rec build_expr builder (te : S.t_expr) =
    ...
    | (S.SFunc_Lit(func), typ)      -> build_func builder func
  ...
  and build_print_call t_el builder =
    ...
    let to_print_texpr texpr = match (snd texpr) with
        A.Bool_t  ->
          ...
          let () = ignore ( build_stmt builder (S.SIf(texpr,
  ...
  and build_if builder predicate then_stmt else_stmt =
  ...
  (* have to handle main specially so program will run *)
  and build_main builder main typ =
    let f = (fst main) in
    let {formals = f.sf_formals; rt = f.sf_return_t; body = f.sf_body } = f in
    let ftype = ltype_of_typ typ in
    let func = L.define_function "main" ftype the_module in
    build_fbody builder func f

  (* variable declaration, handling functions *)
  and build_vdecl builder vd =
    let { init = vd.sv_init; name = vd.sv_name; typ = vd.sv_type } = vd in
    match name with
        "main"  -> build_main builder init typ
      | _       ->
          let init_val = build_expr builder init in
          match typ with
              A.Func_t (_, _) ->
   ...
   and  build_stmt builder = function
    ...
    | S.SBlock(sl) -> ignore(List.map ( build_stmt builder) sl)
    ...
    | S.SVdecl(sval_decl) -> build_vdecl builder sval_decl
        let init_val = build_expr builder sval_decl.sv_init in
    ...
    | S.SIf (predicate, then_stmt, else_stmt) ->
        build_if builder predicate then_stmt else_stmt
