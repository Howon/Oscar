...
exception Builtin_arg_type_err of string * (sexpr list)
...
let empty_func = SExpr (SNoexpr, Unit_t)
...
let find_func (f_name : string) (param : ((sfunc list) option * expr option) list) (env : scope)  =
  List.find (fun sf ->
    sf.sf_name = f_name &&
      check_args_t (get_list_snd sf.sf_formals) param
  ) env.funcs

let find_funcs (f_name : string) (env : scope)  =
  List.filter (fun sf -> sf.sf_name = f_name) env.funcs
...
let funcs_exists (f_name : string) (env : scope) =
  try
    let _ = find_funcs f_name env in true
  with
    _ -> false
...
let rec check_expr (e : expr) (env : scope) =
  match e with
    ...
    | Access(e1, e2) ->
        let (se1, t1) = check_expr e1 env and
          ...
          (match t1, t2 with
            ...
            | Set_t st, _ when types_equal st t2 -> (SAccess(se1, se2), st)
    ...
    | List_Lit (lt, ex) ->
        let te_list = check_expr_list ex in
    ...
    | Message_Lit (m, ex) ->
        let smessage = find_message m env in let te_list = check_expr_list ex in
    ...
    | FuncCall (f, args) ->
        let p_args = List.map (fun a ->
          match a with
              Id id when funcs_exists id env ->
                let f_matches = find_funcs id env in
                (Some f_matches, None)
            | _ -> (None, Some a)
        ) args in
        (try
          check_builtin f p_args
        with
            Builtin_arg_num_err (b, e, num) ->
              raise (Failure ("Builtin function " ^ b ^ " called with too " ^
                "many args: Expected " ^ string_of_int e ^ " but got " ^
                  string_of_int num))
          | Builtin_arg_type_err (b, args) ->
              raise (Failure ("Builtin function " ^ b ^ " cannot be called
                with arguments of type " ^ str_types_list (get_list_snd args)))
          | Not_found ->
              (try
                let sf = find_func f_name p_args env in
                  if check_func_call sf args then
                    (SFuncCall(f, check_expr_list args), sf.sf_return_t)
                  else
...
and check_expr_list (el : expr list) =
...
and check_stmt (s : stmt) (env : scope) =
  match s with
    ...
    | Actor_send (e1, e2)->
        let (se1, st1) = check_expr e1 env and (se2, st2) = check_expr e2 env in
          (match st1, st2 with
...
and check_builtin (f : string) (args : expr list) =
  let se = check_expr_list args in
  let args_len = List.length se in
    match f with
        "reverse" ->
          if (args_len <> 1) then
            raise (Builtin_arg_num_err (f, 1, args_len))
          else
            let (_ :: tail) = se in
              let (_, tail_t) = tail in
                match tail_t with
                    List_t _ | Set_t _ | Map_t (_, _) -> (SCall(f, se), tail_t)
                  | _ -> raise (Builtin_arg_type_err (f, se))
      | "size" ->
          if (args_len <> 1) then
            raise (Builtin_arg_num_err (f, 1, args_len))
          else
            let (_ :: tail) = se in
              let (_, tail_t) = tail in
                match tail_t with
                    List_t _ | Set_t _ | Map_t (_, _) -> (SCall(f, se), Int_t)
                  | _ -> raise (Builtin_arg_type_err (f, se))
      | "contains" ->
          if (args_len <> 1) then
            raise (Builtin_arg_num_err (f, 2, args_len))
          else
            let (head :: tail) = se in
              let ((_, head_t), (_, tail_t)) = (head, tail) in
              match tail_t with
                  List_t t | Set_t t | Map_t (t, _) when
                    types_matc head_t = t -> (SCall(f, se), List t)
                | _ -> raise (Builtin_arg_type_err (f, se))
      | "prepend" | "append" ->
          if (args_len <> 2) then
            raise (Builtin_arg_num_err (f, 2, args_len))
            ...
            let (head :: tail) = se in
              let ((_, head_t), (_, tail_t)) = (head, tail) in
                match tail_t with
                    List_t lt when types_matc head_t = lt -> (SCall(f, se), List lt)
                  | _ -> raise (Builtin_arg_type_err (f, se))
      | "popFront" | "popBack" ->
          if (args_len <> 1) then
            raise (Builtin_arg_num_err (f, 1, args_len))
          ...
          let (_ :: tail) = se in
            let (_, tail_t) = tail in
              match tail_t with
                  List_t _  -> (SCall(f, se), tail_t)
                | _ -> raise (Builtin_arg_type_err (f, se))
      | "filter" -> raise Not_found
      | "foldLeft" -> raise Not_found
      | _ -> raise Not_found
