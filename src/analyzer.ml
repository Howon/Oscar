...
let check_func_call (f_entry : sfunc) (params : t_expr list) =
    ...
    List.iter2 (fun (t1, t2) -> match t1, t2 with
        ...
        | _ -> if (t1 != t2) then raise Exception.Type_mismatch else
                    (fun a -> ())
    ) actual_params required_params;
    (SCall(f_name, params), fentry.sf_return_t)
...
let check_actor_decl (sa_entry : sactor) (params : t_expr list) =
    ...
    else (SActor_Lit(sa_entry.sa_name, params), Actor_t(sa_entry.sa_name))
...
let rec check_expr (e : expr) (env : environment) =
    match e with
        ...
        | Actor_Lit(at, e) ->
            let actor = find_actor env at in
            let te_list = List.map (fun exp -> check_expr e env) e in
            check_actor_decl sactor te_list
        | Pool_Lit(at, e, int) ->
            if (e < 1) then
                raise (Failure ("Pool must spawn at least one actor"))
            else
                let actor = find_actor env at in
                let te_list = List.map (fun exp -> check_expr e env) e in
                check_actor_decl sactor te_list
